...
const llamar = async (categoria) => {
    ...
    const response = await fetch(`https://biblioteca-backend-tau.vercel.app//api/favoritos/${parseInt(id_usuario)}`);
    ...
    const insertarDatos = (datos) => {
        ...
        fetch(`https://biblioteca-backend-tau.vercel.app/api/libros?nombreLibro=${nombreDelLibro}&categoria=${categoria}`)
        .then(res => res.json())
