...
const BuscarUsuarioPorEmailBD = (req, res) => {
    ...
    db.query('SELECT * FROM usuarios WHERE usuarios.email = ?', [email], async (err, result) => {
        if (err) {
            ...
            if (result.length === 0) {
<<<<<<< HEAD
                res.status(200).json({ err: "error de validación de usuario" })
            } else {
...
>>>>>>> main
                if (!isValid) {
                    res.status(404).json({ err: 'no se encontró el usuario' });
                } else {
...
=======
                    ...
                    .cookie('access_token', token, {
                        ...
                        secure: process.env.NODE_ENV === 'produccion', // la cookie solo se puede acceer en https
                        sameSite: 'strict', // la cookie solo se puede acceder en e lmismo dominio
                        maxAge: 1000 * 15 * 60 // la cookie tiene una tiempo de validez de 1 hora
                        // maxAge: 1000 * 60 * 60 // la cookie tiene una tiempo de validez de 1 hora
                    }) 
                    .json(userDatos);
                };
